...
//get value
static int func_and_get_value(corewar_t *core, type_t *arg,
...
// check argument
static bool and_check_arg(type_t args[])
{
    ...
    type_t args[], arg_t *proc_arg)
{
    ...
    if (number_args != 3)
        return 0;
    if (and_check_arg(args) == false)
        return 0;
    val[0] = func_and_get_value(core, &args[0], registers, proc);
    val[1] = func_and_get_value(core, &args[0], registers, proc);
