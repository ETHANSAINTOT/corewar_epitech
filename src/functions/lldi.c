...
// check arg
static bool lldi_check_arg(type_t args[])
{
...
static int get_value_arg(corewar_t *core, type_t *arg, arg_t *proc_arg)
{
    if (arg->type == T_IND)
        ...
        (arg->int_val)) % MEM_SIZE);
    else if (arg->type == T_REG)
        return ((arg->int_val) % MEM_SIZE);
    else
        return ((arg->short_val) % MEM_SIZE);
    return 0;
}
    ...
    type_t args[], arg_t *proc_arg)
{
    ...
    if (number_args != 3 || !lldi_check_arg(args))
        return 0;
